...
#include "include/repair_neurite.hpp"
...
//std::string c_simulated_position(std::string json_info, NumericMatrix new_nodes, NumericVector is_bifurcation, bool omitapical, bool omitaxon, bool omitdend, bool correct, bool remove_zjumps) {
// [[Rcpp::export]]
List c_simulated_position(std::string json_info, NumericMatrix new_nodes, NumericVector is_bifurcation, int init_id, bool omitapical, bool omitaxon, bool omitdend, bool correct, bool remove_zjumps) {
  ...
  /*for(auto s_it= r->begin()->begin_soma(); s_it!=r->begin()->end_soma();++s_it){
    Rcout<<"El nodo del soma tiene ID: "<<s_it->id()<<std::endl;
  }*/
  delete p;
  ...
  for(auto n_it = r->begin(); n_it != r->end(); ++n_it){
    ...
    if(remove_zjumps) repair_neuron(n);
