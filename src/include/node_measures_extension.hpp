...
// Get fractal dimension of the branch from the node to the root of the branch
/*const auto node_fract_dim_to_branch_root = [](const Node& n) -> float {
  if(n.valid_branch())
  {
    ...
    tmp_branch->insert(tmp_branch->begin(),n.branch().root());
    tmp_branch->insert(tmp_branch->end(),n);
    ...
    if(it->get()!=nodes.end())
    {
      nodes.erase(it->get()+1,nodes.end());
    }
    return(node_set_fractal_dim(nodes.begin(),nodes.end()));
  }else{
    return(std::numeric_limits<float>::quiet_NaN());
