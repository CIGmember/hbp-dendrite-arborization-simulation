...
{
  neurostr::Node prueba(4307);
  auto my_node=std::find(it->begin_node(),it->end_node(),prueba);
  if (my_node != it->end_node())
  Rcout<<"Prueba:"<<my_node.node()->parent().id()<<std::endl;
}*/
...
/*
    for(auto it = n.begin_neurite(); it != n.end_neurite(); ++it){
      auto f=get_node_measures_by_id(*it,markers);
      m.emplace_back(f);
    }*/
